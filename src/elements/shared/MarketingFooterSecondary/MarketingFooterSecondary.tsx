...
import classes from "./MarketingFooterSecondary.module.css";
import legacy from "/public/short-logo.svg";
import discord from "/public/logo/discord-s.svg";
import yt from "/public/logo/yt-s.svg";
import twitter from "/public/logo/twitter-s.svg";
import insta from "/public/logo/insta-s.svg";
import fb from "/public/logo/fb-s.svg";
import Link from "next/link";

const MarketingFooterSecondary = () => {
  return (
    <>
          ...
          <Box className={classes.container}>
            <section className={classes.legacy}>
              ...
              <Typography
                ...
                fontWeight="400"
              >
                ™ & © 2022 Legacy Suite, Inc. Legacy Suite is not a law firm and
      ...
      <Box sx={{ backgroundColor: "#092B5A" }}>
        <Container>
          <Box className={classes.secondaryFooterContainer}>
            <Box sx={{ display: "flex" }}>
              <Link href="/terms-of-use">
                <Typography variant="p" color="neutral.lighter">
                  Terms of Use
                </Typography>
              </Link>
              <Link href="/privacy-policy">
                <Typography variant="p" color="neutral.lighter" pl="22px">
                  Privacy Policy
                </Typography>
              </Link>
            </Box>
            <Box
              sx={{ display: "flex", gap: "12px", mt: "15px", mb: "10px" }}
              className={classes.iconContainer}
            >
              {imageData.map((data, index) => (
                <Box key={index}>
                  <Link href={data.link} key={index}>
                    <Image src={data.image} key={index} alt="logo" />
                  </Link>
                </Box>
              ))}
            </Box>
...
export default MarketingFooterSecondary;
